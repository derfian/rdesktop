...
/* Process a TS_LOGON_INFO_EXTENDED structure */
static void
process_logon_info_extended(STREAM s)
{
	...
	if (fieldspresent & LOGON_EX_AUTORECONNECTCOOKIE)
	{
		...
		if (len != 28)
		{
			logger(Protocol, Error,
			       "process_save_session_infotype_logon_extended_info(), "
...
process_save_session_info_pdu(STREAM s)
{
	...
	switch (infotype)
	{
		...
		default:
			logger(Protocol, Warning,
			       "process_save_session_info_pdu(), unhandled info type 0x%x",
			       infotype);
			break;
	}
